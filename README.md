...
wall clock). The problem is stated in your own units and `N` sets the resolution — it is the one knob, and
it buys accuracy with time. Run it as is after `pip install peclet matplotlib`, or open it as a notebook in
...
> measured on the wheel above, this 2.5-second solve did not finish in **15 minutes** on 2 CPUs of
> quota with 48 visible, and took **2.5 s** with the pool bounded. Before importing peclet:
>
> ```python
> import os
...
> ```
>
> The Colab notebook resolves this process's own cgroup and takes the tightest quota on the chain
> — `/sys/fs/cgroup/cpu.max` is the cgroup *root*, and reads `max` on a host that does limit you.
