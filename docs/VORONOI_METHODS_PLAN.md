...
the moving mesh of track D. The collocated variant stores the velocity vector at seeds and uses the
ABC approximate projection: average the cell velocities onto the faces (distance-weighted along the
connector, C1's `projectToFaces`), make that FACE field divergence-free with the same two-point
Laplacian `L`, then correct the cell velocities with the cell-centred pressure gradient (C1's
Green–Gauss) — the same staggered-vs-collocated pair `flow` carries
(`docs/studies/staggered_vs_colocated.md`), so the two variants are gated against each other on
every C case, and the collocated one needs no Rhie–Chow compatibility term at all. The remaining mesh-quality error is **skewness** (facet centroid off
...
| **C2b Collocated projection (ABC)** | seed-centred velocity vector; per step: predictor on the cell velocities (Green–Gauss gradients from C1), average to faces (`projectToFaces`), face projection with the same `L` and PCG (the divergence-free face field is the flux the transport uses), cell correction with the Green–Gauss pressure gradient — `flow`'s ABC approximate projection on the Voronoi face mesh, no Rhie–Chow; one `FlowSolver` with a `layout` switch, as `flow` has | the same TGV gates as C2a; covolume vs collocated compared on TGV, Poiseuille and C4 at matched seeds (order, checkerboard freedom, cost per step) — a `docs/studies/` page | M |
