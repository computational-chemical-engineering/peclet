...
| **C3 Body-fitted boundaries** — **no-slip / prescribed wall velocity LANDED 2026-09-03** (`setWallVelocity` on both solvers, `test_body_fitted`) | wall faces of the SDF-clipped cells carry the prescribed velocity: the constraint `T` returns `U_wall·n` on them, the viscous wall flux is the two-point `ν A (U_wall − U_i)/h_A`, the convective wall flux carries `U_wall`, the pressure is Neumann (flow's structure: the wall enters the constraint and the momentum with one operator family). Slip / inflow-outflow patches: follow-up. | **Measured:** Poiseuille between SDF slabs, body force, cubic lattice with the walls halfway between seed rows — the interior Stokes residual of the exact parabola is round-off (4e-16), the wall row's is exactly f/4: the two-point wall flux is the derivative at h_A/2, not at the wall, so the profile is NOT exact — the plan's "exact to round-off" was wrong. The steady state converges at **order 2.00** for both solvers (relative error 8.5e-2 / 2.1e-2 / 5.3e-3 at 4/8/16 cells across the gap), wall flux 0, divergence 3e-16; on the lattice the two solvers coincide to the digit for this flow. A quadratic wall flux (exact for the parabola) is a possible follow-up; flow's cut-cell IBM is second order too. Engine facts found: the tessellator's periodic gather needs every box extent > 2× its coverage radius (a 0.25-thin box segfaults the cell builder — documented in the test); the degenerate cubic lattice produces zero-area non-reciprocal facets that the face mesh now drops (`nDropped`) instead of treating as walls; the pressure CG needs a zero-RHS guard (0/0 on a quiescent start). | S |
| **C4 Cross-code gate** — **MEASURED 2026-09-03, target OPEN** (`test_permeability`) | Stokes drag of the simple-cubic sphere array (φ = 0.216, Zick–Homsy K = 7.442) on the body-fitted Voronoi mesh: 0.15h-jittered lattice seeds (a cubic lattice around a sphere is degenerate and the clipper overflows — 20–117 overflowed cells, non-deterministic volumes), seeds within 0.4 h of the wall dropped, cells clipped by the sphere SDF (fluid volume exact to 2e-5), collocated solver, Stokes, tight steady-state stop | **K error −13.4 % at 12 cells per diameter, −7.5 % at 18 (order ≈ 1.4)** vs flow's cut-cell IBM −0.5 % at N = 32. Diagnosis: the wall cells are fat and irregular (h_A up to ~1.4 h) and the two-point wall flux is a first-order wall shear — geometry exact, wall gradient not. Remedies identified: (i) wall-adapted seeding (a seed shell at h/2 from the surface = the grid generator's boundary layer) — blocked today by the 64-plane cell cap `kMaxP` (131–298 overflowed cells, fluid volume wrong); (ii) a second-order one-sided wall gradient on the wall faces (flow's `gpCenterGrad` / wall-anchored quadratic idea, which is exactly what makes flow's cut cells accurate). Also found: `kIncomplete` is raised on every wall cell facing the sphere (the unclipped extent exceeds the gather coverage) — expected, the volume identity is the validity check. Gate as measured (valid meshes, error decreasing, ≤ 10 % at n = 24); the 1 % / 3 % cross-code target and the packed bed stay open behind (i)/(ii). | M |
| **C5 MPI + Python** — **Python half LANDED 2026-09-03** (`peclet.voro.FlowSolver(tessellation, viscosity, layout='collocated'|'covolume', amg=True)`: `set_body_force`, `set_stokes`, `set_skew_corrected`, `set_wall_velocity`, `set_velocity`, `step`, `get_velocity`/`get_pressure`/`get_cell_volume`, `kinetic_energy`, `max_divergence`, `pressure_iterations`; `python/test_voro.py::test_flow_solver` — TGV decay on a jittered 16³ lattice: collocated E/E0 within 0.23 % of exact, covolume 1.7 %, face divergence 1e-14) | **MPI half OPEN:** 2-ring `VoronoiHalo`, distributed `L` through core's halo, distributed GraphAMG (the memory's open item) — np = 1, 2, 4 bit-exact to single rank (the flow standard) | M |
