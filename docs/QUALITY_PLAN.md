...
| umbrella | `full.log`; `.gitignore` the `flow-*`/`tel/` worktrees and `*.log`; prune the 13 merged flow worktrees (~12 GB) once their sessions close. **DONE 2026-09-22:** `69503dc29254db50bfdf4f41/` (an Overleaf clone of the *Robust Scaled IBM Solver* paper, the hash being its project id) → `~/Codes/paper-robust-scaled-ibm-solver`, clean and in sync with its remote, which still carries the id; and `.sdf-campaign-probes/` → `flow/tests/study/sdf_campaign/` and now TRACKED (flow `8789e2b`) — nine documents cited those gates and their measured numbers while a fresh clone had none of them; the six captured `.bin`/`.npy`/`.npz` artifacts stay gitignored. **DONE 2026-09-08:** `sphere-cfd-validation/` (its own git repo) and `proposal/` moved to `~/Codes/`; voro's PR-required branch protection removed so all eight repos have the same (none); `morton/octree/` removed, its 151-line std::map octree kept as core's test-local oracle (`core/tests/oracle/`) | |
