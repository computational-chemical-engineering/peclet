...
| W3 | **run on Snellius 09-04** (WO-W3) | the block container is now checkpointable BITWISE (`vof_block_colour`/`enable_vof_blocks_from_colours` + `P` + both WY parity counters); `channel_18` at 106 ms/step on one H100 — but the run dies at ~1.5 eddy turnovers when two markers interpenetrate, independent of dt: **the block CSF sums two markers' forces (UNPACK_SUM) on faces where the projection sees one union `max` colour, so the balanced-force pairing cannot hold** → **W4 (collision/coalescence, and a balanced-force block CSF for overlapping markers) is on the critical path**; TBFsolver built and run on Snellius (0.069 s/step on 64 Genoa cores) — its converged `channel_18` statistics are the FIRST reference for the case (centreline ⟨α⟩ 0.034, peak ⟨u⟩/u_τ 16.3 at y/h 0.81); on a matched early window peclet's bubbles have not dispersed and the near-wall turbulence decays, because flow's cubic cells force `dy⁺ = 3.18` where TBFsolver runs 1.59 — **an anisotropic-cell limitation of `flow`, not of VoF**. Cost: 0.96 GPU-h + ~470 core-h |
| W4–W5 | W4 **PARKED 09-05** as a WIP commit on flow branch `vof-w4` (f29c8e7): union-colour CSF on the overlap faces, pair census (film, approach velocity, We_c), `set_block_coalescence` (never/film/weber), breakup by in-block component labelling, event log — last local battery 32/33 (`vof_blocks` failing), the W3 checkpoint still dies at 1.53 turnovers (gate G2 NOT met); **not in the 0.7.0 release**, colliding bubbles are outside the block container's rating | W4 resumes after the release; W5 unchanged |
