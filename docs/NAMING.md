...
| — (new) | `Flow.set_pressure_pcg(on)` — flow's name and tier. Two deliberate differences: `on=False` selects amr's bounded stationary V-cycle (flow raises: it has no non-Krylov fallback), and there are no `max_iter`/`rtol` arguments (amr has them as `pres_iters` and `set_pressure_tolerance`; a second spelling of each is what §1 forbids) | **canon** (amr `16ec7b7`, 2026-09-24) |
