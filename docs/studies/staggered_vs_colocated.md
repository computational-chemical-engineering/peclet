...
!!! note "Source"
    Generated by `flow/tests/study/staggered_vs_colocated.py`; the canonical copy lives in the flow
    submodule at `flow/doc/history/colocated_study/`. Reproduce with that script's `run` / `report` modes.
