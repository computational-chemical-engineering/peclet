...
!!! note "Source"
    Generated by `sdflow/tests/study/staggered_vs_colocated.py`; the canonical copy lives in the sdflow
    submodule at `sdflow/doc/colocated_study/`. Reproduce with that script's `run` / `report` modes.
