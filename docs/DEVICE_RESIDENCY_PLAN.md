...
- **flow** — `Solver::step()` (diffusion smooth + cut-cell projection + MG-PCG/Chebyshev) is
...
- **voro** — the tessellation CSR stays device-resident within a step; all physics
...
- **GridHalo** (`grid_halo.hpp`) is the reference pattern: field stays on device, only the compact
  `nSend/nRecv` buffers cross, with a GPU-aware path (`PECLET_CORE_GPU_AWARE_MPI`) that hands device pointers to
...
voro's *unused* incremental updater, the *pnm* pipeline, and a recurring redundant host-copy idiom.
...
- **S1 — Device CSR-fill primitive** (`peclet::core::` device util): the canonical
...
- **S2 — Zero-copy / single-copy ndarray export helper** in the `peclet::core::python` bridge: (a) a
  ...
  (dem, voro, flow/pnm); (b) an opt-in DLPack / `__cuda_array_interface__` **device** export so a
...
### Theme D — Particle migration on device (core ⇒ dem, voro)
...
### Theme E — voro incremental update (wire up existing device code)
...
| Item | What | Where | Effort |
|------|------|-------|--------|
| E1 | **DONE** (voro `3baf127`, opt-in scaffolding). Wire incremental update into `ExplicitEuler::buildAndForce` instead of full rebuild | `device_simulation.hpp` | L |
...
### Theme F — pnm extraction pipeline (flow) — one-shot, but big host work
...
| Item | What | Where | Effort |
|------|------|-------|--------|
| G1 | Replace the redundant *mirror → element loop → vector* idiom in all binding getters with **S2** | `dem/src/sim.hpp:461-483`, `vorflow_bindings.cpp:97-128`, `flow pore_extraction.hpp` | S |
...
| G4 | flow: move `setSolid`/`uploadVelocity` single-rank periodic ghost wrap to device (mirror the distributed inner-fill + device `fillGhosts`) | `sdflow_ibm.hpp:196,128` | S–M (cold) |
...
- **H1** — `PECLET_CORE_GPU_AWARE_MPI` exists in GridHalo but is opt-in/host-staged by default. Make GPU-aware
...
- **H2** — DLPack / `__cuda_array_interface__` device export for `get_*` accessors across flow, dem,
  voro on the shared bridge (**S2b**), so GPU-resident Python analysis chains avoid D2H entirely.
...
- **Phase 0 — quick wins (low risk):** ✅ **DONE** (umbrella 774199b, 2026-06-29). S2a helper, then
  ...
  redundant-copy idiom suite-wide and the step-invariant H2D in voro. Per-repo commits:
  core b976351 (S2a `peclet::core::toVector`, C1 gather-plan memoization, D3 ParticleHalo scratch +
  reverse-slice, G5 fused fv removeMean, G6 one-D2H `velocities()`), morton 45be8c7 (G7 `MORTON_HD` on
  comparisons / `wide_uint` / range queries), dem 635b13f (G1 getters, G2 dup pair-count), voro
  3a843b9 (G1 getters, E3 `WorklistCache`, E4 viscous scratch + upload-mass-once), flow 7c25b9c
...
- **Phase 2 — voro incremental update (Theme E):** **E1 DONE** (voro `3baf127`, opt-in). The
  fluid step (`ExplicitEuler::buildAndForce`) can now use the moving-point repair
...
- **Phase 3 — distributed device compute + device migration (Themes C, D):** C2, D1, D2, then H1 and at-
  ...
  5c70aa1):** `ParticleHalo::gather` reuses the owner↔ghost topology under a Verlet skin (build with
  ...
  346bf0f):** the device distributed gather halo gained the `PECLET_CORE_GPU_AWARE_MPI` device-pointer-MPI path
...
- **Phase 4 — pnm + device outputs (Themes F, H2):** F1/F2, H2. Lower frequency, real but one-shot.
  **DONE.** F3 landed in Phase 0; F2 (flow `26290b0`) replaces the host `std::map` segmentation
  ...
  F1 (flow `d793ae5`) splits each pnm stage into a device core + thin host wrapper and adds the fused
  ...
  flow/voro. Caveat: a device-export array holds a ref-counted View, so release it before the
  module's atexit `Kokkos::finalize` (hence opt-in).

## Validation & correctness invariants

- Every device port keeps its host oracle and is checked against it (AMR geometric operators; voro
