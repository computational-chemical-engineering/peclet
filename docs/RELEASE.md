...
| **Docs**: umbrella MkDocs site, per-repo Doxygen, Python API pages | GitHub Pages | `site.yml` (umbrella, on push to main touching docs/), `docs.yml` per repo; `tools/gen_python_api.py` regenerates `docs/python/*.md` | works; the Python pages are regenerated by hand (§5.1); the site pages carry no version literals (§5.2) |
...
- A3. Scratch hygiene. The umbrella working tree carries untracked campaign directories
  (`flow-*` worktrees; `sphere-cfd-validation/` and `proposal/` moved to `~/Codes/` on 2026-09-08, and on
  2026-09-22 `.sdf-campaign-probes/` into `flow/tests/study/sdf_campaign/` and the hash-named paper repo
  to `~/Codes/paper-robust-scaled-ibm-solver`, leaving the worktrees as the only untracked scratch).
...
**Resolved 2026-09-04:** experiment 1 (run 33868865327) made flow, dem, voro link; core's `amr` and
voro's wrappers then failed for virtual classes in anonymous namespaces (named namespaces fixed it);
run 33874788583 built the full image. The rule for new binding code: virtual wrapper classes go in a
named namespace. The list below is kept for the record.
...
2. **GitHub Releases**: one per repo at its new tag + the umbrella, body = the CHANGELOG section
   ...
   `CITATION.cff` do not change; `CITATION.cff` carries only the concept DOI (a version DOI there goes
   stale — Zenodo and each GitHub release link the version's own).
3. **Sites**: the umbrella `Site` workflow runs on the docs push; each repo's `docs.yml` on its
   push. Check that the PyPI badge on `https://computational-chemical-engineering.github.io/peclet/`
   shows the new version (shields.io caches for a few minutes) and that the Python API pages carry
   the new methods (§5.1 regenerates them; there is no version to edit on the site).
