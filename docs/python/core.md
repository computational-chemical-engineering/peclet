...
| `cells` | Finest-level cell counts per axis (root*2**lmax). |
...
| `extent` | Block side lengths in world units (cells*spacing). |
...
| `refine_to_sphere` | refine_to_sphere(self, center: collections.abc.Sequence[float], radius: float, target_level: int = 0, band: float = 1.0, balance: bool = True) -> int  Refine leaves the sphere surface passes through (plus `band` cells) down to target_level; optionally restore 2:1 balance. Returns the number of refinements performed. |
...
| `cells` | GLOBAL finest-level cell counts per axis (global root cells * 2**lmax). |
...
| `extent` | Global box side lengths in world units (cells*spacing). |
...
| `origin` | Global lower corner in world coordinates. |
| `rank` | This process's MPI rank. |
...
| `sizes` | sizes(self, axis: int = 0) -> numpy.ndarray[dtype=float64]  Local leaf world widths along `axis`, (num_leaves,) float64 (see Octree.sizes). |
| `spacing` | Finest cell size (dx, dy, dz), per axis. |
