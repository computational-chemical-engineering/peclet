...
|-----------|------------------|--------------|-------------------|
| `core/` | Header-only C++20 + MPI | **Shared infrastructure**: ORB block decomposition + asynchronous ghost-layer exchange (NBX + persistent engines) + particle migration + SDF geometry + dynamic load balancing. The layer every method code depends on. (It also still carries the AMR octree + solver under `amr/`; no method code uses them, and QUALITY_PLAN G.2 moves that whole tree to a new `peclet-amr` package before 1.0.0.) Tested (109 plain / 164 Kokkos / 7 Python ctests, np 1–8). | **Yes — read it** |
