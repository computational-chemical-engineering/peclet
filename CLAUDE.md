...
- [docs/DECISIONS.md](docs/DECISIONS.md) — the **decision register**: what the project chose, what it rejected, and why, for all 580 decisions (521 in force, 59 superseded), with verbatim quotes and provenance in [docs/decisions/](docs/decisions/). It exists because settled decisions were being silently reversed — a later session picking the textbook alternative because nothing in front of it said the project had already rejected that alternative on purpose. Each submodule's `CLAUDE.md` inlines its own highest-risk prohibitions. **Read the entry for anything you are about to change; reversing one takes a new recorded decision, not a judgement call in the moment.**
...
| `geom/` | Header-only C++20, nanobind, **no Kokkos and no MPI** (Python `peclet.geom`) | **Analytic-SDF scene authoring**: CSG trees over signed-distance primitives, batch evaluation, lattice baking, and rigid-body mass properties (mass, centre of mass, inertia tensor, principal frame). Host-only — its closure is six `peclet/core/geom/*.hpp` headers, which STAY in `core` and are vendored here at `PECLET_CORE_TAG`. Split out of `peclet-core` in peclet 1.2.0 so a pure-geometry API no longer needs an MPI toolchain to install (`docs/CORE_BOUNDARY.md`); `peclet.core.geom` still works through the `peclet-core` compatibility shell until 2.0.0. | No |
